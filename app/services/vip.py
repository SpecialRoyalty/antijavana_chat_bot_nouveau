...
async def _group_for_offer(offer:str)->int|None:
    ...
    if role:
        gid=await chat_id_for_role(role, include_unavailable=False)
        if gid:
            return gid
    return None
...
async def validate_vip(bot:Bot, order_id:int, ok:bool):
    async with SessionLocal() as db:
        ...
        if ok:
            for offer in [x for x in order.offers.split(',') if x]:
                group=await _group_for_offer(offer)
    ...
    if not ok:
        ...
        return 'OK'
    # Total et JAVANA : lien immédiat. Pass soirée : immédiat uniquement
    # entre 23h et 5h ET pendant une vraie session ouverte. Si A+B = AUCUN,
    # la commande reste en attente pour la prochaine ouverture.
    ...
    async with SessionLocal() as db:
        for aid in accesses:
            ...
            if not a: continue
            if a.offer=='soiree' and not soiree_live:
                waiting.append(aid)
            else:
...
async def send_due_pass_soiree_links(bot:Bot, force:bool=False):
    # À 23h, uniquement si une vraie session est ouverte. Aucune ouverture = aucun lien.
...
async def copy_media_to_vip(bot:Bot,msg:Message):
    ...
    from app.services.multigroup import chat_id_for_role, ROLE_VIP_SOIREE, ROLE_VIP_TOTAL, ROLE_VIP_JAVANA
    targets=[
        (await chat_id_for_role(ROLE_VIP_SOIREE, include_unavailable=False),'soiree'),
        (await chat_id_for_role(ROLE_VIP_TOTAL, include_unavailable=False),'total'),
        (await chat_id_for_role(ROLE_VIP_JAVANA, include_unavailable=False),'javana'),
    ]
